...
public:
    ...
    // point for odd and traversing to the adj list to see wether we can find
    // the palindromic string
    //  similarly we are taking two midpoint one i and its adjacent node and
    //  checking
    // wether we can find palindromic
    ...
    int maxLen(int n, vector<vector<int>>& edges, string label) {
        ...
        int ans = 1;
        cache.resize(n + 1,
                     vector<vector<int>>(n + 1, vector<int>(1 << 14, -1)));
        ...
        for (int i = 0; i < n; i++) {
            ...
            for (int j = 0; j < adj[i].size(); j++) {
                if (label[adj[i][j]] == label[i]) {
                    ans = max(ans, 2 + solve(adj, i, adj[i][j],
                                             0 | (1 << i) | (1 << adj[i][j]),
                                             label));
                }
            }
    ...
    int solve(vector<vector<int>>& adj, int node1, int node2, int mask,
              string& label) {
        if (cache[node1][node2][mask] != -1) {
            return cache[node1][node2][mask];
        }
        int ans = 0;
        for (int i = 0; i < adj[node1].size(); i++) {
            ...
            for (int j = 0; j < adj[node2].size(); j++) {
                ...
                if ((mask >> newNode2) & 1 || newNode1 == newNode2)
                    continue;

                if (label[newNode1] == label[newNode2]) {
                    ans = max(
                        ans, 2 + solve(adj, newNode1, newNode2,
                                       mask | (1 << newNode1) | (1 << newNode2),
                                       label));
                }
            }
        }
